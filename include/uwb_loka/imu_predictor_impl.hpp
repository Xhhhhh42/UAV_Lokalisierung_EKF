#ifndef UWB_LOKA_IMU_PREDICTOR_IMPL_H
#define UWB_LOKA_IMU_PREDICTOR_IMPL_H
...
IMU_Predictor::IMU_Predictor( State_Ptr &state_ptr, double acc_n, double gyro_n, double acc_w, double gyro_w )
    : state_p_( state_ptr ), 
      acc_noise_( acc_n ), 
      gyro_noise_( gyro_n ), 
...
bool IMU_Predictor::init( double ts_meas ) 
{ 
    return inited_ = init( *state_p_, ts_meas, last_imu_ptr_ ); 
}


/// @brief 将 IMU 数据加入到 IMU 缓冲区中，并在需要时移除异常值。同时根据系统是否已初始化来决定是否执行加入缓冲区的操作
/// @param imu_ptr 
/// @param inited EKF是否初始化
/// @return 
bool IMU_Predictor::push_data( ImuData_Ptr imu_ptr, const bool &inited ) 
{
...
/// @param last_imu_ptr 
/// @return 
bool IMU_Predictor::init( State &state, double ts_meas, ImuData_ConstPtr &last_imu_ptr ) 
{
    // 检查IMU缓冲区中的数据是否足够进行初始化，如果不足，则输出警告消息
    if ( imu_buf_.size() < kImu_BufSize ) {
        ROS_INFO_STREAM_NAMED( "IMU_Predictor", "Initialization ERROR: Not Enough IMU data for Initialization!!!" );
        return false;
    }
    ...
    if ( std::abs( ts_meas - last_imu_ptr->timestamp ) > 0.05 ) {
        ROS_WARN_STREAM_NAMED( "IMU_Predictor", "Initialization ERROR: timestamps are not synchronized!!!" );
        return false;
    }
    ...
    // Set bias to zero.
    state.set_bias( Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero() );
...
/// @param R 
/// @return 
bool IMU_Predictor::init_rot_from_imudata( const std::deque<ImuData_ConstPtr> &imu_buf, Eigen::Matrix3d &R ) 
{
    ...
    // ROS_INFO_STREAM_NAMED( "IMU_Predictor-Function : Init_rot_from_imudata", 
 ...
 */
void IMU_Predictor::propagate_state( ImuData_ConstPtr last_imu, ImuData_ConstPtr curr_imu, const State &last_state, State &state, 
                           bool with_noise, bool with_const_noise, const Eigen::Matrix<double, kNoise_dim, 1> &vec_noise ) 
{
    ...
    const Eigen::Vector3d new_posi = last_state.get_position() + last_state_vec.segment<3>(0) * dt + 0.5 * acc_nominal * dt2;
 ...
 * @param state
 */
void IMU_Predictor::propagate_state_cov(ImuData_ConstPtr last_imu, ImuData_ConstPtr curr_imu, const State &last_state, State &state ) 
{
    const double dt = curr_imu->timestamp - last_imu->timestamp;
...
Eigen::Matrix<double, kState_dim, kState_dim> IMU_Predictor::noise_cov_discret_time( double dt ) 
{
...
Eigen::Matrix<double, kNoise_dim, kNoise_dim> IMU_Predictor::noise_cov( double dt ) 
{
    const double dt2 = dt * dt;
...
#endif // UWB_LOKA_IMU_PREDICTOR_IMPL_H
